...
- ~~**Four systems assume exactly one `Camera3d` exists, with no in-code marker of that assumption**~~ _(observed during local co-op Stage 3 planning, 2026-07-05, before any split-screen code was written; confirmed as a real, reproduced bug 2026-07-08; `world_label_screen_pos_system` fixed 2026-07-10 — see `planning/features/world_label_split_screen_positioning.md`)_ — originally `world_label_screen_pos_system` (`lib.rs:508`), `nameplate.rs:212`, `particle_renderer.rs:303`, and `targeting.rs:122` each queried `With<Camera3d>` and either `.single()` (graceful no-op on 2+ matches) or `.iter().find(...)` (arbitrarily picks one camera). **`world_label_screen_pos_system` is now viewport-aware** — it iterates every active `Camera3d`, picking the one whose own `logical_viewport_rect()` contains the projected point (deterministic `SplitViewportSlot` ordering), fixing positioning for every `WorldLabel` consumer (room labels, entity labels, stat labels, damage popups, nameplate anchors) in one pass. **Still affected**: `nameplate.rs:212`'s distance-culling, `particle_renderer.rs:303`'s billboard orientation, and `targeting.rs:122`'s click-to-select — none crash, but they still silently no-op or arbitrarily pick one of the two cameras in a split-screen scene. Worth the same viewport-aware treatment if/when a project actually combines split-screen with nameplates, particles, or click-targeting; `world_label_screen_pos_system`'s selection logic (`world_to_viewport` + `logical_viewport_rect().contains()`) is the reference pattern to follow, though each site's query shape/result usage differs enough that a shared helper wasn't worth extracting yet (deferred until a second consumer needs the exact same shape).
