...
- [ ] **Per-player targeting for split-screen** — see `planning/features/per_player_split_screen_targeting.md`. `capabilities/targeting.rs`'s `CurrentTarget` is one shared global resource (not per-player), and `tab_targeting_system` hardcodes `controllers.iter().next()` — it always acts on whichever `CharacterController` is found first, never player 2's, regardless of that player's own `target_next` keybinding. `click_select_system`'s viewport-aware fix (Phase 2 of `planning/features/done/split_screen_camera_followups.md`) correctly resolves *which entity* a click in a given viewport should hit, but the result still overwrites the one shared `CurrentTarget` — so player 1 and player 2 fight over a single target, and a real single-mouse-in-split-screen limitation (only one viewport can ever have the cursor over it at once) means mouse-driven targeting can't be simultaneous for both players regardless. Tab-cycling is the one approach that could give both players independent targeting at once (each already has their own keys via `InputMap`), but needs `CurrentTarget` and `tab_targeting_system` made per-player first. _Surfaced 2026-07-12 during the split-screen click-to-select playtest._
