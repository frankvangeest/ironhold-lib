...
### Camera
- [ ] **Split-screen: remaining single-camera assumption sites** — `world_label_screen_pos_system` was made viewport-aware for both scene-level `world_labels:` and per-entity `label:` (room names), see Done log, but four other sites still assume exactly one active `Camera3d` and no-op or arbitrarily pick one once split-screen has 2+: `nameplate_visibility_system`'s distance-culling (`camera_q.single()`, fails every frame in every split-screen scene, Stage 3+), `particle_renderer.rs:303`'s billboard orientation (unconditional fallback, not just an edge case), `targeting.rs:122`'s click-to-select, and the `WorldLabelRank` multi-viewport-duplication fix — which does not yet extend to stat labels, world stat bars, damage popups, or nameplate anchors, so those stay single-instance (rank 0 only) and don't duplicate across two simultaneously-visible split viewports. `world_label_screen_pos_system`'s selection pattern (`world_to_viewport` + `logical_viewport_rect().contains()`) is the reference to reuse for the first three; `WorldLabelRank` is the reference for the fourth. See `planning/features/split_screen_camera_followups.md` (Draft) and `planning/claude_suggestions.md` ▸ Camera.
