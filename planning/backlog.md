...
- [ ] **Unify prefab feature application across all spawn paths** — move `attach_prefab_features` to `entity_spawner.rs` and call it from the tail of `spawn_prefab_instance`, removing the six duplicate feature blocks there; any new `PrefabDef` capability field then only needs one wiring point. See `planning/features/unify_prefab_feature_application.md`.
