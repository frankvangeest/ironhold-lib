...
- [ ] **ToggleMute from pause overlay does not affect live audio sink** — indicator and `GlobalVolume` update correctly but the already-playing background music keeps its old volume; only the playing state's `ToggleMute` path patches the live sink. Reproduce: `3rd_person_game_demo`, game starts muted; open pause overlay and click Toggle Mute → indicator shows unmuted but no sound plays; close overlay and click Toggle Mute again → sound toggles correctly. Suspected cause: `bg_music_query` in `action_executor.rs` lines 264–304 does not cover the sink spawned by `PlayMusicLoop`; `GlobalVolume` change takes effect at sink creation only, not on live sinks.
