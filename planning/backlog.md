...
- [ ] **Consolidate entity-spawn component insertion** — shared `tag_spawned_entity` helper so all ~7 spawn sites attach `SpawnId`/`PrefabKey`/`LevelEntity`/registry + markers identically; fixes GLB-player (no SpawnId/PrefabKey/registry) and dynamic-spawn (no PrefabKey/LevelEntity) gaps. See `planning/features/spawn_site_consolidation.md`
