...
### Engine / Runtime
- [ ] **Intent event layer** — emit `intent.slot.{n}:{entity}` from `action_bar.rs` before committing, route through the interpreter; designers can then cancel/redirect ability slots from RON rules using `when:` gates. Fixes the one capability that bypasses the Message→Interpreter→Action→Executor pipeline. Do before authoring combat abilities. See `planning/features/intent_event_layer.md`.
