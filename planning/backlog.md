...
- [ ] **Push `stat_overrides` into `spawn_prefab_instance`** — `scene_loader.rs` builds a second `StatMap` insert for the GLB actor path that relies on last-write-wins; add `stat_overrides: &HashMap<String, f32>` parameter to `spawn_prefab_instance`, move override-aware construction there, and remove the redundant second insert from scene_loader. From `claude_suggestions.md`.
- [ ] **Add `collider_radius`/`collider_height` to `NpcDef`** — GLB NPC capsules are fixed at 0.35 m / 1.6 m in `entity_spawner.rs`; add optional fields with those defaults following the same pattern as `MovementConfig`. From `claude_suggestions.md`.
- [ ] **Integration test: GLB Actor `components.npc` attaches `NpcAgent` + `LocomotionState`** — the new `spawn_prefab_instance` GLB NPC path added in `df8c94b` is untested; add a test that spawns `enemy_snake` and asserts both components are present on the spawned entity. From `claude_suggestions.md`.
