...
- [ ] **Unify prefab feature application across all spawn paths** — `attach_prefab_features` (scene_loader.rs, Primitive paths) and the six-feature block in `spawn_prefab_instance` (entity_spawner.rs, GLB Actor/Prop path) are still two independent lists; have `spawn_prefab_instance` call `attach_prefab_features` at its tail and remove the duplicate blocks; after this any new `PrefabDef` capability field only needs one wiring point. See `planning/features/unify_prefab_feature_application.md`.
