...
- [ ] **Embed capability shaders & fix hardcoded shared asset paths** — `stat_radar`, `foliage`, `flame_material`, `particle_renderer` shaders are runtime file-loaded; embed via `include_str!()` so the library works without `assets/shared/`; fix `foliage.rs` fabricated texture fallback path. See `planning/features/embed_capability_shaders.md`
