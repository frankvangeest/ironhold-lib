...
- [ ] **Mute audio toggle + master volume** — `AudioConfig` on `ProjectConfig` with `max_volume: f32` (project ceiling, default 1.0) and `mute_on_start: bool`; `ToggleMute` and `SetVolume(f32)` actions; `AudioState` resource; `GlobalVolume` = `active_fraction * max_volume` (or 0 when muted); `audio.muted` / `audio.unmuted` / `audio.volume_changed` pipeline events; mute button demo in `3rd_person_game_demo`. See `planning/features/audio_mute_toggle.md`
