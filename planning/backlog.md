...
- [ ] **Per-instance overrides on `SceneEntityDef`** — add `scale`, `label`, and `stat_overrides` as `#[serde(default)]` `Option` fields to the placed-instance entry in scene RON; no schema version bump, all existing RON stays valid; eliminates the need to fork prefabs for trivial per-placement variations (e.g. `shrine_burning` / `shrine_cold` / `shrine_default` / `shrine_inactive` collapse to one prefab with overrides). Highest-value, lowest-disruption gap from the scene/prefab boundary analysis.
