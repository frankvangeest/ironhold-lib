...
- [ ] **Per-player stat pools (action-bar costs)** — see `planning/features/per_player_stat_pools.md` (Ready — plan reviewed by system-architect + ux-gamedesigner-reviewer 2026-07-17). Scope narrowed from the original "13 files" estimate: reuses the existing `StatMap`/`PrefabDef.stat_templates` mechanism NPCs already have (zero new RON schema) by forwarding `stat_templates` onto player prefabs and rewiring the 3 `SlotCost` read/write sites in `action_bar.rs` to check the acting player's own pool first, falling back to the global `LoadedStats` exactly as today when absent — fully backward compatible. Inventory/dialogue currency remain out of scope (neither system exists in any split-screen project yet).
