...
- [ ] **Action bar custom hotkeys** — bind any `parse_key`-recognised key name (`"KeyQ"`, `"KeyE"`, `"F2"`) to action bar slots in RON; removes the hardcoded `DIGIT_KEYS` table; new `key_hint` field overrides the on-screen corner glyph (distinct from the existing `label` field); fully backward-compatible with existing `"1"`–`"9"` layouts. Hard dependency for per-player action-bar execution (Phase 2, `planning/features/per_player_split_screen_targeting.md`) — ships first so two split-screen players' bars can use disjoint hotkeys. See `planning/features/action_bar_custom_hotkeys.md`.
