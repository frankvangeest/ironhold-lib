...
- [ ] **Per-player stat/resource pools** — `LoadedStats` is a single global resource; the action bar's `SlotCost` check/deduct and the cooldown-overlay dim both read it directly, so in split-screen two players' action bars visually look per-player but silently share one invisible pool (spending on one bar dims the other). Touches 13 files (`stats.rs`, `action_bar.rs`, `action_executor.rs`, `player.rs`, `inventory.rs`, `dialogue.rs`, `stat_radar.rs`, `stat_display.rs`, `schema/stats.rs`, others) — a genuinely per-player economy (per-entity `StatMap`-backed costs, not just the existing `"{self}.stat"` NPC addressing) is a substantial cross-cutting change, not a quick prerequisite. Surfaced 2026-07-16 during per-player action-bar execution (Phase 2) planning; deliberately deferred rather than folded in — no current project needs independent per-player resource economies. Needs its own plan file before starting.
