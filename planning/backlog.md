...
- [ ] **Split-screen: remaining single-camera assumption sites** — `world_label_screen_pos_system` was made viewport-aware for both scene-level `world_labels:` and per-entity `label:` (room names), see Done log, but four other sites still assume exactly one active `Camera3d` and no-op or arbitrarily pick one once split-screen has 2+: `particle_renderer.rs:303`'s billboard orientation (unconditional fallback, not just an edge case — fix first), `targeting.rs:122`'s click-to-select, `nameplate_visibility_system`'s distance-culling (`camera_q.single()`, fails every frame in every split-screen scene, Stage 3+ — resolved via store-and-read from `world_label_screen_pos_system`'s own selection rather than reselecting), and the `WorldLabelRank` multi-viewport-duplication fix extended to stat labels + Ascii world stat bars (both scene-placed and dynamically-spawned via `Action::Spawn`), gated on the scene actually being split-screen to avoid a 4x per-frame cost in ordinary single-player scenes; Pixel-style bars, damage popups, and nameplate anchors stay deferred (child-hierarchy duplication cost). Plan-reviewed by system-architect + ux-gamedesigner-reviewer (2026-07-11), verdict Ready — phase order and the two Frank-resolved decisions above came out of that review. See `planning/features/split_screen_camera_followups.md`.
