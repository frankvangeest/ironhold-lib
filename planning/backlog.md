...
### Rendering & Assets
- [ ] **Deferred rendering** — replace clustered forward with Bevy's deferred pipeline to remove the `MAX_FADING_LIGHTS = 16` cap and efficiently handle large numbers of dynamic lights (torches, particle lights, explosions); transparent/additive materials (particles, decals) stay on the forward path automatically. Investigation complete — Bevy 0.18 uses `Rgba32Uint`/`R8Uint` G-buffer (standard WebGPU formats); all custom materials default to forward automatically; MSAA replaced by FXAA. **Ready for feature file after live WASM browser test passes.** See `planning/investigations/deferred_rendering_spike.md`
