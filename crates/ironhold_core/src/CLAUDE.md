...
- **`NameplateTag`** — inserted at spawn time by `scene_loader.rs` when `show_nameplates: true` or `prefab.nameplate == Some(true)` (and not suppressed by `Some(false)`). `nameplate_setup_system` queries `Added<NameplateTag>` every `Update` frame and spawns the anchor + `Text2d` + pixel bar quads for any newly-tagged entity — scene-placed entities, dynamically spawned actors, and wave-spawned enemies all use the same path. Bar fills are driven by the existing `world_pixel_bar_update_system`. `NameplateSceneConfig` (resource) is populated from `GameSceneV2` on each scene load and cleared on `LoadScene`.
