...
- **`NameplateTag`** — inserted at spawn time when `should_insert_nameplate(prefab.nameplate, show)` returns true (`scene_manager/mod.rs`, beside `tag_spawned_entity`): `nameplate: Some(false)` always suppresses; otherwise `show` (`scene.show_nameplates` for scene-placed entities, `nameplate_config.enabled` for `Action::Spawn`) or an explicit `nameplate: Some(true)` opt-in enables it. This is the single source of truth for all 5 nameplate-gating call sites (`scene_loader.rs` ×4, `entity_spawner.rs` ×1) — do not re-inline the predicate at a new call site. `nameplate_setup_system` queries `Added<NameplateTag>` every `Update` frame and spawns the anchor + `Text2d` + pixel bar quads for any newly-tagged entity — scene-placed entities, dynamically spawned actors, and wave-spawned enemies all use the same path. Bar fills are driven by the existing `world_pixel_bar_update_system`. `NameplateSceneConfig` (resource) is populated from `GameSceneV2` on each scene load and cleared on `LoadScene`. **Known gap:** the character-select player spawn path (`action_executor.rs`, `Action::Spawn` for a player-tagged prefab) does NOT go through `should_insert_nameplate` — it only checks `nameplate != Some(false)`, ignoring `show_nameplates` entirely. See `planning/backlog.md` Bugs.
