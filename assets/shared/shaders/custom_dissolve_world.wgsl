...
// Procedural dissolve — world-space variant. Seamless on any geometry.
//
// Uses world-space XZ position for the noise input so the dissolve pattern
// has no UV seam on spheres, cylinders, or organic shapes.
//
// Use custom_dissolve_uv.wgsl when UV-aligned dissolve is preferred (e.g. the
// cutout should follow the mesh's texture layout on flat/box geometry).
//
...
//     "tiling"       → params_2.z            — noise scale in world units (0.5–2.0)
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let noise = fbm(in.world_position.xz * tiling);
