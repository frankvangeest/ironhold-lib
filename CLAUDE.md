...
for t in fsm_tests entity_logic_tests scene_lifecycle_tests spawn_tests action_tests npc_tests \
         ...
         gamepad_binding_tests player_slope_jump_tests prop_ground_veto_tests \
         corpse_loot_interact_tests wall_friction_tests; do
  echo "=== $t ==="
...
10. **Mark the feature Done, commit, and merge into `integration`:**
    ...
    - On the feature branch/worktree: mark the item Done in `planning/backlog.md` by **physically relocating its bullet into the `## Done (reference)` section**, filed under the current `### <Month> <Year>` subheading (add one if the month doesn't have one yet) — for a feature, out of `## Active`; for a bug fix, out of `## Bugs`. Checking `[x]` in place and leaving the bullet where it was is **not** sufficient and does not count as done — a bullet with `[x]` still sitting in `## Bugs`/`## Queued`/`## Icebox` is exactly the drift that let 16 stale entries accumulate there before a 2026-09-16 cleanup caught it. Also move the matching `planning/features/{name}.md` (if one exists) into `planning/features/done/`, then commit (code + tests + docs — never `pkg/`).
...
14. **Commit `pkg/` on `integration`, in its own commit — never combined with any other change** (code, docs, `planning/backlog.md`, agent-memory, or anything else, even a one-line unrelated fixup). Use `git add -f pkg/` (not plain `git add`): `pkg/.gitignore` is a blanket `*`, so any *new* filename `wasm-pack` emits (as opposed to ones already tracked) would otherwise be silently skipped. Keeping every `pkg/` commit isolated is what makes it safe to later squash, rebase, or purge a run of stale `pkg/` build commits without touching any other change's history — see `planning/CLAUDE.md`'s hash-citation note for why this matters (a `pkg/`-only history purge on 2026-09-03 required remapping every hash citation in the repo because commits weren't cleanly separable at the time). **Never cite a `pkg/` commit's hash in any planning markdown** (`planning/backlog.md`, a feature file, `claude_suggestions.md`) — always cite the nearest code/docs-only commit instead (typically the feature branch's own Done-marking commit, made well before any `pkg/` build happens). If you ever need "the commit this batch's `pkg/` build corresponds to," describe it in words (e.g. "the `pkg/` commit immediately following `<hash>`"), don't cite its hash directly.
