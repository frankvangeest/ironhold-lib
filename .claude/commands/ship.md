Walk through the ironhold-lib code change workflow for the current feature. Execute each step in order, report its status, and do not skip steps.
...
6. **Docs updated** — Check that `docs/20_data_formats.md` and any relevant `CLAUDE.md` files reflect the changes. New schema fields, new action types, and new events each need a doc entry. Also check `crates/ironhold_core/src/CLAUDE.md` for capability-level notes.

7. **UX review** — If any files in `assets/`, `docs/`, or schema RON files changed, invoke the `ux-gamedesigner-reviewer` agent to verify the designer experience is clear, documented, and consistent.

8. **Schema/CLI check** — If any file in `crates/ironhold_core/src/schema/` was modified, run:
   ```
...
9. **WASM dev build** — Run:
   ```
   wasm-pack build crates/ironhold_web --target web --out-dir ../../pkg --dev
   ```
   Report the size of `pkg/ironhold_web_bg.wasm`. Warn if ≥ 95 MB.

10. **Play-test checklist** — Provide a concrete checklist for Frank to verify the feature in the browser: which project to load, what to interact with, what to look for. Include golden path and at least one edge case.

11. **Await play-test confirmation** — Stop here. Do not proceed to step 12 until Frank explicitly confirms the feature works in the browser.

    If Frank reports a bug or regression during play-testing:
    - Return to **step 3** (implement the fix)
    - Re-run **steps 4 → 5 → 6 → 7 → 8 → 9** (alignment review, tests, docs, UX review, schema check, dev WASM build)
    - Then return to **step 10** (updated play-test checklist) and **step 11** (await confirmation again)
...
13. **Move feature to Done** — In `planning/backlog.md`, mark the item `[x]` and move its feature file from `planning/features/` to `planning/features/done/` if one exists.

14. **Commit** — Stage all changed files and commit with a descriptive message in conventional-commit format. Include a summary of what changed and why.
