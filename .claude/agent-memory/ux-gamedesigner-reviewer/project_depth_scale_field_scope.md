...
**Default changed 50.0 → 20.0 (feature/label-depth-scale-validation, 2026-08-30)** to match `entity_spawner::default_camera_config()`'s `max_radius`. Two prose spots in `docs/20_data_formats.md` still cite the old `50.0` default: the nameplate depth-scale paragraph (~line 623) and the nameplate example's RON comment (~line 681). Check both any time the default moves — the field table at ~line 377 gets updated, the prose does not.

**Validation shipped for the two silent failures (same branch):** `min_scale` outside [0,1] = CLI hard error `label_depth_scale_min_scale_out_of_range` + scene-load clamp+`warn!`; `reference_distance` outside the union of reachable camera radii = CLI `--strict` warning `label_depth_scale_reference_distance_outside_camera_range` + scene-load `warn!`. Both message sets are properly prescriptive (name field, state consequence, suggest a value). Known asymmetry: the runtime `warn!` for `reference_distance` does NOT fire when the only player is spawned via `Action::Spawn` (`3rd_person_game_demo`) — only the CLI check covers that. Flycam-only scenes (e.g. `custom_materials`, `reference_distance: 80.0`) are skipped by BOTH checks by design — no signal at all there.
