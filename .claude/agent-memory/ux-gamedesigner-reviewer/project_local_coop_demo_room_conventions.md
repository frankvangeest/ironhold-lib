...
`assets/projects/local_coop_demo/` has grown to 10 rooms (`main`, `room2`–`room10`) with several
...
- **One colour identity per room** — blue/amber/teal/rose/gold/green/violet/crimson (room2-9) and
  **steel/silver-blue (room10)**. Ground prefab + portal accent cylinder + scene `lighting` all
  share the tone. Still free: lime, white. **Always cross-check the hint text against the portal
  prefab's accent cylinder colour** — room10 gets this right ("Red portal -> room 9" vs
  `portal_to_room9`'s `(0.90, 0.15, 0.20)`).
- **Every room gets its own `scene.ready:roomN` Log rule** — room9's was missing and was added
  together with room10's in the player-model-v2 change; the gap is now closed for all 10.
...
- **Top-left hint ladder is `y = 20 + 44n`, `x = 20`, `900x32`** — confirmed across every room.
  room3 and now room9 are tied as the most crowded at 6 rungs (y=20..240). room10 has 5. room6/room8
  show two extra tiers for per-player captions: `y=316` and `y=676`, `600x32`, at `x=20` / `x=660`.
  Flag cumulative footprint once a room passes ~5 rungs.
- **Rung ordering is inconsistent**: room3/room5/room9 put `controls_hint` first, room10 puts
  `room_hint` first. room10's order (what room am I in → controls → thesis → detail) reads better;
  worth standardizing on it.
