...
**Gamepad `just_pressed` in FixedUpdate is a latent hazard (input.rs:126/137).** `input_translator_system` runs in **FixedUpdate** and reads `gp.just_pressed(GamepadButton::South/East)` for jump/run. Because FixedUpdate can run 0 or 2+ times per render frame, a gamepad `just_pressed` edge (a per-frame flag, same as keyboard) can be **missed or double-counted** across the schedule boundary — a real pre-existing bug, not yet observed/filed. Correct pattern for any NEW gamepad edge-trigger (e.g. gamepad-routed action-bar slots): read the live `Query<(Entity, &Gamepad)>` and `just_pressed` **in Update**, mirroring `camera_orbit_system` (Update, input.rs analog-stick *value* reads are fine in FixedUpdate; it's only edge-triggered `just_pressed` that's hazardous). `action_bar_input_system` is already in Update and already reads keyboard `just_pressed` there — adding gamepad `just_pressed` to it is consistent and correct. **A headless gamepad test harness now exists** — `crates/ironhold_core/tests/gamepad_binding_tests.rs` (and gamepad support in `tests/support/mod.rs`, which runs the real `gamepad_connection_system`/`gamepad_event_processing_system` against `MinimalPlugins`) — this claim of "zero `Gamepad` refs / no harness" is stale; see [[gamepad_input_pattern]] for the harness details.
