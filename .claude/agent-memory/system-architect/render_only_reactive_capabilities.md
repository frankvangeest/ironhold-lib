...
Canonical example: `capabilities/target_indicator.rs` (`TargetIndicatorPlugin`), added ~2026-06. Ground-ring decal that tracks `CurrentTarget`.

Established conventions for this class of capability:
- It does NOT push to `ActionQueue`. It only reads state resources (`CurrentTarget`, `LoadedTargetIndicator`, `SpawnRegistry`) and spawns/moves/despawns a visual. Correct — it's not a gameplay-logic source.
