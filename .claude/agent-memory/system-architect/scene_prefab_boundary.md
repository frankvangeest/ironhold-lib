...
1. **Per-instance overrides on SceneEntityDef — PARTIALLY ADDRESSED.** `SceneEntityDef` now carries `stat_overrides: HashMap<String, f32>` (`#[serde(default)]`, `schema/scene_v2.rs`) — exactly the additive, no-version-bump override channel recommended in "Recommended direction" #1 below, but scoped to overriding a stat's initial `base` value from the prefab's `stat_templates` only (min/max/regen/thresholds unchanged). This is a real per-instance override, but a narrow one — it does NOT generalize to material/motion/other prefab fields yet, so the broader "no per-instance overrides" gap (and the prefab-fork evidence below) still stands for anything other than stat starting values. Evidence in repo: `shrine_*` ×4 (identical geometry, diff orb color+hint), `attack_dummy`/`attack_dummy_ascii` (diff only in world_stat_bar.style), `chest_01`/`chest_02`.
...
1. **Per-instance overrides on SceneEntityDef** — start additive (`material: Option<String>`, `motion: Option<MotionDef>`, etc., `#[serde(default)]`, NO version bump). `stat_overrides: HashMap<String, f32>` has already landed as one instance of this pattern (see above) — extend the same additive-field approach to `material`/`motion`/etc. rather than inventing a different mechanism. General `overrides:` block later (needs feature file + likely GameSceneV2 v3).
