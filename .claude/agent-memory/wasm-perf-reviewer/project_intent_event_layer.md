...
**Per-player Phase 2 change (feature/per-player-action-bar-execution, reviewed 2026-07-16):** the anticipated restructure landed — `find(...)` → `slots.iter().filter(|s| s.resolved_key.is_some_and(|kc| keys.just_pressed(kc)))` loop. **Idle cost is IDENTICAL to the old `find`**: on a zero-key-pressed frame `find` cannot short-circuit (no match), so both scan all N slots running the same `Copy`-KeyCode predicate; the loop body never enters. N = slots per scene (single-digit × a few bars). No new per-frame alloc. New system param `players: Query<(&SpawnId, &PlayerTarget, Option<&PlayerIndex>), With<CharacterController>>` — archetype state is cached across frames, so adding the query costs ~nothing on idle frames; `players.iter().find(owns_slot(...))` only runs inside the fired-slot branch, O(player count) capped at MAX_SPLIT_PLAYERS=4. All allocs (`format!`, `key_str.to_string()`, `do_actions.clone()`+`rewrite_target` Vec, HashMap insert) stay inside the per-press branch. `is_primary_player` (targeting.rs) = `player_index.map_or(true,|i| i.0==0)`, trivial. Verdict: OK for web, no regression. The only behavior delta (filter processes all fired slots vs find stopping at first) is intentional and only manifests on multi-press frames.

Related: [[project_rewrite_target]] (rewrite_target canonical copy lives in message_interpreter.rs), [[project_per_player_targeting]] (Phase 1 PlayerTarget component), ActionQueue FIFO ordering preserved (flush pushes in stored Vec order).
