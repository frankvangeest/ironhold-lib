...
**Stage 4 (~2026-07-05): `Horizontal` split arm added.** `split_screen_viewport_system` gained a `SplitOrientation::Horizontal` match arm (splits `physical_height` top/bottom, screen-space-Y-down so slot 0 = top at y=0, remainder absorbs odd-pixel rounding). Arithmetically identical to the `Vertical` arm — one division, one subtraction, two UVec2 constructions; NO new allocations/queries/deps/shaders. Same `ActiveSplitScreen`-gated early-return applies, so still free on all non-split scenes. `SplitOrientation` enum went from `Vertical`-only to `Vertical`+`Horizontal` (+`Dynamic` still reserved). New RON in local_coop_demo (room4.scene.ron, 2 prefabs, portal/rules). No frame-time or binary-size regression.
...
**Measured pkg/ironhold_web_bg.wasm = 58 MB (existing artifact dated 2026-07-05, from a prior build — not necessarily rebuilt for Stage 4).** Well under 95 MB warn / 100 MB block. This matches the user's auto-memory clean-release figure; treat as clean-build reference.
