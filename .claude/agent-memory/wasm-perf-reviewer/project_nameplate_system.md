...
- `nameplate_visibility_system` — runs every frame, iterates `Query<(Entity,&NameplateTag,&NameplateAnchor)>` (~8 entities). `npc_q.contains(entity)` is **O(1)** (Bevy `Query::contains` is archetype/sparse-set membership, NOT O(log n)). Writes to Visibility are change-detection-guarded (`*vis != Hidden`). Cheap. Early-returns if `config.options` is None.
  - **As of Phase 3 split_screen_camera_followups (feature/nameplate-distance-culling, 2026-07-12):** this system no longer queries `Query<&GlobalTransform, With<Camera3d>>` and no longer reads entity GlobalTransform. It reads the per-anchor distance stashed by `world_label_screen_pos_system` via `dist_q.get(anchor.0)` (O(1) random-access). Net **slightly cheaper** than before (dropped a camera single() + per-entity transform read, added one O(1) get). Distance is now against whichever camera actually *positioned* the anchor (split-screen correctness), not an independently re-selected one. Ordering is load-bearing: registered `.after(world_label_screen_pos_system)` (lib.rs ~L308) so the stash is same-frame, no lag. New component `NameplateCameraDistance(Option<f32>)` — trivial size.
