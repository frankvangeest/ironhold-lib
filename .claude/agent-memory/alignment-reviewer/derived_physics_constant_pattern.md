...
description: uphill_jump_lock (v2 grace / v3 slope limit / v4 coyote time / v5 sensor exclusion) — the three-tier derive-vs-expose-vs-invariant rule for physics values, the single CharacterController construction site, and the can_jump branch-exclusivity footgun that makes coyote time silently suppress double jump
metadata:
  type: project
---
