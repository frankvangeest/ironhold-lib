...
**Panel click-blocking / backdrop follow-up reviewed clean (2026-06-28):** ALIGNED — matches the [[guard-vs-behavior-distinction]] hybrid the system-architect pre-approved same day.
  ...
  - WARNINGS at intro: (1) the six bool+emit sites are hand-duplicated — recommend a `set_panel_open(scene_state, game_events, bool)` helper; `ToggleInventory` is the fragile one (computes opened-vs-closed from resulting state). (2) `ui.panel_opened`/`ui.panel_closed` have NO shipped example rule (grep: only action_executor.rs) and NO docs — designers can't discover them. Same discoverability gap as [[intent-event-layer-pattern]].
  - `GlobalZIndex(51)` on the 3 panel spawns (scene_loader ~1856/2032/2156) is ACCEPTED, not a magic-number violation: same engine-owned UI-stack convention as the pre-existing overlay backdrop `GlobalZIndex(100/101)` in the same file; base-scene UI is implicit 0; no `z_index` field exists anywhere in schema/ (grep confirmed), so introducing one would be the inconsistency. Designer authors panel content/position, not UI draw order.
