...
- No shipped example project demonstrates an intent rule (demo task left unchecked in feature file). Only coverage is `entity_logic_tests.rs` (rule-handled SetVariable case — file was `integration_tests.rs` before the 2026-07-02 test-file split). WARNING-level: designers have docs but no copyable working project rule.
