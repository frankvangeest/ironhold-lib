...
- `rules_path`/`state_machine_path` are resolved the same way, but with **no** convention-path fallback when unset — the runtime's own fallback for an unset `rules_path` is inline `ProjectConfig.rules` (never a guess at `logic/rules.ron` on disk), and an unset `state_machine_path` means no state machine at all. A project setting only one of the two no longer has the other's dead convention-path file silently treated as live by any check (a real bug this closed: two shipped projects' unrelated `logic/rules.ron` was previously counted as coverage by the `orphan_rule` check even though the runtime never loads it). This fallback-free behavior only applies once a `.project.ron` exists — with none at all, `validate` still falls back to both convention paths, same reasoning as the catalog paths above.
- Every path reference above that passes its `exists()`/`is_file()` check (scene paths, dialogue paths, prefab behavior paths, the five configured catalog paths, and `rules_path`/`state_machine_path`) is also checked for exact case and forward-slash separators against the real on-disk file (`path_case_mismatch`) — `Path::exists()` is case-insensitive and `\`-tolerant on Windows/NTFS, so `"Scenes\\Main.scene.ron"` validates clean locally while 404ing over HTTP in the actual WASM/browser build. Author every path with forward slashes and the file's exact on-disk casing. A mis-cased configured path is still parsed afterward (unlike a genuinely missing one), so this doesn't hide other checks that depend on that file — except `rules_path`/`state_machine_path` at the project.ron-attributed-inline-rules branch, which has no file to mis-case in the first place.
...
- A convention-path logic file (`logic/rules.ron` / `logic/state_machine.ron`) exists on disk but its matching `.project.ron` field (`rules_path` / `state_machine_path`) is unset (`unset_logic_path_with_convention_file`) — unlike the catalog warning above, there is no fallback checking it either: the file gets zero signal from any tool at all until this fires
