...
| Interactable entities       |   ✅   | `Interactable { radius, hint_text, requires_item }` component; when player is within `radius` metres and presses the interact key (`inputs.interact` on the player prefab, default `"KeyF"`), emits `entity.interacted:{id}` — or, if `requires_item` names an `items.ron` key the player's `PlayerInventory` lacks, `entity.interact_blocked:{id}` instead. Add via `interactable` field on `PrefabDef`. |
| Motion (rotate/bob)         |   ✅   | `Motion` component; world-space continuous rotation (per-axis rad/s) and sinusoidal vertical bob (amplitude, frequency). Runs in `FixedUpdate` (64Hz, moved from `Update` in `deterministic_fixed_timestep.md` v1 — a motion-carrying entity can also carry a collider). |
...
- `Action::SetEntityVisible { entity: String, visible: bool }` — shows or hides a spawned entity by its spawn ID; the entity stays in the ECS (stats, colliders, and behavior FSM keep running); world labels (stat bar, stat label, and per-entity `label:`) tracking the entity auto-hide; `{self}` substituted in behavior files
...
- `entity.interact_blocked:{id}` — emitted instead of `entity.interacted:{id}` when `interactable.requires_item` names an `items.ron` key the player's `PlayerInventory` doesn't hold (never both)
